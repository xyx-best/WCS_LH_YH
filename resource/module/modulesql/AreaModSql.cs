...
namespace resource.module.modulesql
{
    public class AreaModSql : BaseModSql
    {
        ...
        public List<Area> QueryAreaList()
        {
            ...
            string sql = string.Format("SELECT t.id, t.`name`, t.`enable`, t.devautorun, t.c_sorttask, t.memo, full_qty FROM area AS t ");
        ...
        {
            string sql = "UPDATE `area` set name = '{0}', `enable` = {1}, `devautorun` = {2}, `memo` = '{3}' where id = '{4}'";
            sql = string.Format(sql, area.name, area.enable, area.devautorun, area.memo, area.id);
            int row = mSql.ExcuteSql(sql);
            return row >= 1;
        }
