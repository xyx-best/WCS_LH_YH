...
namespace resource.track
{
    public class TrackMaster
    {
        ...
        public int GetTracCode(ushort trackid, uint ferryid)
        {
            int code = 0;
        ...
        internal List<Track> GetRecentGoodTracks(List<AreaDeviceTrack> list, uint devid)
        {
        ...
        public bool IsTrackFree(uint trackid)
        {
            ...
            return track != null && track.TrackStatus != TrackStatusE.停用 && track.StockStatus == TrackStockStatusE.空砖;
        }
        ...
        public bool HaveTrackInGoodButNotStock(uint areaid, uint tilelifterid, uint goodsid, uint currentTake, out uint trackid)
        {
            ...
            //1.查看是否有最近下砖规格轨道
            List<Track> recentusetracks = GetRecentGoodTracks(devtrack, tilelifterid);
